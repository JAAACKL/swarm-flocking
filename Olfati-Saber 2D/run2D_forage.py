...
if __name__ == '__main__':
    ...
    runSimulation(25, with_obstacles=True, is_flocking=False) 
    ...
    food_nf = np.zeros((num_runs, max_steps))
    food_f  = np.zeros((num_runs, max_steps))
    speed_nf = np.zeros(num_runs)  # Array for no-flocking final average speeds
    speed_f  = np.zeros(num_runs)  # Array for flocking final average speeds
    ...
    for i in range(num_runs):
        ...
        counts_nf = G_nf.food_remaining_history
        food_nf[i, :len(counts_nf)] = counts_nf[:max_steps]
        # Store final average speed
        speed_nf[i] = G_nf.getFinalAverageSpeed()

        print(f"Trial {i+1}/{num_runs} — Flocking")
        G_f = runSimulation(N, with_obstacles=True, steps=max_steps, is_flocking=True)
        counts_f = G_f.food_remaining_history
        food_f[i, :len(counts_f)] = counts_f[:max_steps]
        # Store final average speed
        speed_f[i] = G_f.getFinalAverageSpeed()

    # Adjust each trial's food remaining to be relative to the final average speed of each trial
    food_nf = food_nf / speed_nf[:, np.newaxis]
    food_f  = food_f / speed_f[:, np.newaxis]

    # Compute mean and std over trials for food
    mean_nf = food_nf.mean(axis=0)
    std_nf  = food_nf.std(axis=0)
    mean_f  = food_f.mean(axis=0)
    std_f   = food_f.std(axis=0)
    ...
    ax.set_title('Food Remaining Over Time')
    ax.set_xlabel('Timestep')
    ax.set_ylabel('Unit of Food Remaining')
    ax.legend()
    ax.grid(True)
    plt.show()
